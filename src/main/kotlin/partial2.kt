...
private fun mainIO(args: Array<String>): IO<Unit> = fx {
    putStrLn("What is your name?").bind<Unit>()
...
private fun gameLoop(name: String?): IO<Unit> = fx {
    ...
    getStrLn().safeToInt().fold(
        ...
        {
            val number = nextRandomInt(5).map { n -> n + 1 }.bind()
...
private fun nextRandomInt(upper: Int): IO<Int> = IO { random.nextInt(upper) }
